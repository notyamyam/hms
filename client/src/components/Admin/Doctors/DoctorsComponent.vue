<template>
  <div class="flex flex-col space-y-3">
    <div class="flex justify-between">
      <div>
        <h1 class="font-semibold text-xl text-gray-600">Doctors</h1>
      </div>
      <div>
        <button
...
export default {
  ...
  computed: {
    ...
    filteredPatients() {
      return this.users.filter((user) => user.user_Type === "Doctor");
    },
  },

  methods: {
    goToAddDoctor() {
      this.$router.push("/admin/addDoctors").then(() => {
        window.location.reload();
      });
    },

    goToEditDoctor(userId) {
      this.$router
        .push({ name: "editDoctors", params: { id: userId } })
        .then(() => {
          window.location.reload();
        });
    },

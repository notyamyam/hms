<template>
  <div v-if="user.id" class="flex flex-col space-y-4">
    <div class="flex justify-between">
      <div>
        <h1 class="font-semibold text-xl text-gray-600">Edit this Doctor</h1>
      </div>
      <div>
        <button
...
export default {
  ...
  methods: {
    goToDoctors() {
      this.$router.push("/admin/doctors").then(() => {
    ...
    updateUser() {
      ...
      // Include password if it has been changed
      if (this.user.password) {
        updatedUser.password = this.user.password;
      }

      axios
        .put(`http://127.0.0.1:8000/api/users/${userId}`, updatedUser, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
        })
